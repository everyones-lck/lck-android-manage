...
class QuestionListFragment : BaseFragment<FragmentCommunityListBinding>(R.layout.fragment_community_list) {
    ...
    override fun initObserver() {
        ...
        viewLifecycleOwner.repeatOnStarted {
            viewModel.communityReportListPage.collectLatest { data ->
                if (isPost) {
                    ...
                    (data as? PagingData<CommunityWithReportListModel.CommunityReportListElementModel>)?.let {
                        _postListRVA?.submitData(it)
                    }
                }else {
                    initCommentListRVAdapter()
                    (data as? PagingData<CommentWithReportListResponseModel.CommentWithReportListResponseElementModel>)?.let {
                        _commentListRVA?.submitData(it)
                    }
                }
            }
        }
        ...
        viewLifecycleOwner.repeatOnStarted {
            viewModel.currentFilter.collect { currentFilter ->
                Timber.d("Question", currentFilter)
