...
class FreeAgentListFragment : BaseFragment<FragmentCommunityListBinding>(R.layout.fragment_community_list) {
...
/*    private var readResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){ result ->
    ...
    override fun initObserver() {
        ...
        viewLifecycleOwner.repeatOnStarted {
            viewModel.currentFilter.collect { currentFilter ->
                Timber.d("FreeAgent", currentFilter)
    ...
    private fun initCommentListRVAdapter() {
        ...
        _commentListRVA = CommentListRVA { commentId ->
            val action = DeleteCommunityContentFragmentDirections.actionDeleteCommunityContentFragmentToReadPostFragment(commentId)
            findNavController().navigate(action)
        }
